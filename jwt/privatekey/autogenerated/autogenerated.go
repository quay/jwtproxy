...
const (
	privateKeyPath = "jwtproxy/id.jwk"
)
...
func constructor(registrableComponentConfig config.RegistrableComponentConfig, signerParams config.SignerParams) (privatekey.PrivateKey, error) {
	...
	if basePath == "" {
		basePath = path.Join(os.Getenv("HOME"), ".config")
	}
	keyPath := path.Join(basePath, privateKeyPath)

	var activeKey *key.PrivateKey

	// Load the private key, or leave nil if there was no stored PK.
	storedPrivateKey, err := loadPrivateKey(keyPath)
	if err == nil {
		...
		if err == nil {
			...
			log.Debug("Successfully loaded and verified private key at path: ", keyPath)
	...
	if activeKey == nil {
		...
		publicationResult = ag.attemptPublish(manager, nil, cfg.RotationInterval)
	}

	go ag.publishAndRotate(cfg.RotationInterval, manager, publicationResult)
...
func loadPrivateKey(keyPath string) (*key.PrivateKey, error) {
	...
	pk := key.PrivateKey{}
	jsonDecoder := json.NewDecoder(pkFile)
...
func savePrivateKey(key *key.PrivateKey, keyPath string) error {
	...
	jsonEncoder := json.NewEncoder(pkFile)
...
func (ag *Autogenerated) publishAndRotate(rotateInterval time.Duration, manager keyserver.Manager, publicationResult *keyserver.PublishResult) {
